...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (u.total > 0) {
      ...
      // Upcoming licence renewals -- always shown (unlike other conditional
      // sections) so a "nothing due" result reads as a completed check, not
      // an ambiguous silent omission that looks the same as a broken feature.
      {
        ...
        if (u.licenceRenewalsError) {
          ...
          children.push(callout("No annual licence renewals due in the next 90 days.", "good"));
          children.push(spacer(140));
        }
      }
